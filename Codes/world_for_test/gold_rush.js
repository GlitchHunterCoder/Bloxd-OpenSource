...
function onPlayerJoin(playerId) {
  ...
  if(dev.includes(api.getEntityName(playerId))) {
    ...
    api.broadcastMessage([{
      str: api.getEntityName(playerId) + " Joined the Lobby",
      style: {
        color: "#CEFEFF",
        fontWeight: "0",
        fontSize: "14px",
...
function onPlayerLeave(playerId, serverIsShuttingDown) {
  if(dev.includes(api.getEntityName(playerId))) {
    ...
    api.broadcastMessage([{
      str: api.getEntityName(playerId) + " Left the Lobby",
      style: {
        color: "#CEFEFF",
        fontWeight: "0",
        fontSize: "14px",
...
onPlayerChangeBlock = (playerId, x, y, z, fromBlock, toBlock, droppedItem, fromBlockInfo, toBlockInfo) => {
    if (fromBlock === "Gold Ore") {
 api.createItemDrop(x,y,z, "Gold Fragment", 1, true, {})
    }
    if (fromBlock === "Iron Ore") {
 api.createItemDrop(x,y,z, "Iron Fragment", 1, true, {})
   }
};
onItemDropCreated = (itemId, itemName, x, y, z, initiatorDbId) => {
    if (itemName === "Raw Gold") {
        api.deleteItemDrop(itemId);
}
    if (itemName === "Aura XP Orb") {
       api.deleteItemDrop(itemId);
}
    if (itemName === "Raw Iron") {
       api.deleteItemDrop(itemId);
 }
};
